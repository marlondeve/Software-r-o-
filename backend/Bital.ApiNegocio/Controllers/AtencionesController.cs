...
public class EncuestaCapturaPresencialEnvelope
{
    public IEnumerable<EncuestaCapturaPresencialResponse> Data { get; set; } = Array.Empty<EncuestaCapturaPresencialResponse>();
...
public class AtencionesController : ControllerBase
{
    ...
    /// <summary>
    /// Obtiene la base de pacientes para captura presencial de encuestas
    /// </summary>
    [HttpGet("encuestas/captura-presencial")]
    [ProducesResponseType(typeof(EncuestaCapturaPresencialEnvelope), StatusCodes.Status200OK)]
    [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status500InternalServerError)]
    public async Task<ActionResult<EncuestaCapturaPresencialEnvelope>> GetCapturaPresencial(
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            _logger.LogInformation("GET /api/v1/atenciones/encuestas/captura-presencial");

            var pacientes = await _atencionesService.GetCapturaPresencialAsync(servicio, pabellon, estado, busqueda, cancellationToken);

            var list = pacientes.ToList();
            var response = new EncuestaCapturaPresencialEnvelope
            {
                Data = list,
        ...
        {
            _logger.LogError(ex, "Error al consultar captura presencial de encuestas");
            return Problem(
                title: "Error al consultar captura presencial",
                detail: ex.Message,
                statusCode: StatusCodes.Status500InternalServerError
            );
        }
