...
public class PacientesController : ControllerBase
{
    ...
    /// <summary>
    /// Obtiene el listado de pacientes hospitalizados en una fecha específica
    /// </summary>
    /// <param name="fecha">Fecha de consulta del censo (formato yyyy-MM-dd). Si no se especifica, usa la fecha actual.</param>
    /// <param name="cancellationToken">Token de cancelación</param>
    /// <returns>Lista de pacientes hospitalizados con su ubicación</returns>
    /// <response code="200">Retorna el censo de pacientes hospitalizados</response>
    /// <response code="500">Error interno del servidor</response>
    [HttpGet("hospitalizados")]
    [ProducesResponseType(typeof(ApiResponse<IEnumerable<PacienteHospitalizadoResponse>>), StatusCodes.Status200OK)]
    [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status500InternalServerError)]
    public async Task<ActionResult<ApiResponse<IEnumerable<PacienteHospitalizadoResponse>>>> ObtenerHospitalizados(
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            _logger.LogInformation("Obteniendo censo de pacientes hospitalizados para fecha {Fecha}", fechaConsulta);

            var pacientes = await _pacientesService.ObtenerPacientesHospitalizadosAsync(fechaConsulta, cancellationToken);

            var response = new ApiResponse<IEnumerable<PacienteHospitalizadoResponse>>(pacientes);
            return Ok(response);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error al obtener pacientes hospitalizados para fecha {Fecha}", fecha);
            return Problem(
                title: "Error al obtener censo de pacientes hospitalizados",
                detail: ex.Message,
                statusCode: StatusCodes.Status500InternalServerError
            );
        }
