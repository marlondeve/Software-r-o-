...
public class PacientesQueryService : IPacientesQueryService
{
    ...
    public async Task<IEnumerable<PacienteHospitalizadoResponse>> ObtenerPacientesHospitalizadosAsync(
        DateTime fecha,
        CancellationToken cancellationToken = default)
    {
        _logger.LogInformation("Consultando pacientes hospitalizados para fecha {Fecha}", fecha);

        // Consulta SQL para obtener pacientes hospitalizados activos
        var sql = @"
            SELECT 
                RTRIM(LTRIM(i.MPCedu)) AS PacienteId,
                i.IngCsc AS IdIngreso,
                RTRIM(LTRIM(i.MPCedu)) AS Cedula,
                RTRIM(LTRIM(i.MPTDoc)) AS TipoDocumento,
                RTRIM(LTRIM(ISNULL(c.MPNom1, ''))) + ' ' + 
                RTRIM(LTRIM(ISNULL(c.MPNom2, ''))) + ' ' + 
                RTRIM(LTRIM(ISNULL(c.MPApe1, ''))) + ' ' + 
                RTRIM(LTRIM(ISNULL(c.MPApe2, ''))) AS NombreCompleto,
                DATEDIFF(YEAR, c.MPFchN, GETDATE()) AS Edad,
                RTRIM(LTRIM(ISNULL(c.MPSexo, ''))) AS Sexo,
                RTRIM(LTRIM(ISNULL(i.IngSerCod, ''))) AS Servicio,
                RTRIM(LTRIM(ISNULL(i.HisCamPab, ''))) AS Pabellon,
                RTRIM(LTRIM(ISNULL(i.HisCamNum, ''))) AS Habitacion,
                RTRIM(LTRIM(ISNULL(i.HisCamCam, ''))) AS Cama,
                i.IngFecIng AS FechaIngreso,
                RTRIM(LTRIM(ISNULL(i.IngDxCod, ''))) AS DiagnosticoPrincipal,
                CASE 
                    WHEN i.IngEntSal = 'N' THEN 'Hospitalizado'
                    ELSE 'Egresado'
                END AS EstadoPaciente
            FROM INGRESOS i
            INNER JOIN CAPBAS c ON i.MPCedu = c.MPCedu AND i.MPTDoc = c.MPTDoc
            WHERE i.IngEntSal = 'N'  -- Solo pacientes activos (no egresados)
                AND i.IngFecIng <= {0}  -- Ingresaron antes o en la fecha consultada
            ORDER BY i.HisCamPab, i.HisCamNum";

        var pacientes = await _context.Database
            .SqlQueryRaw<PacienteHospitalizadoDto>(sql, fecha)
            .ToListAsync(cancellationToken);

        return pacientes.Select(p => new PacienteHospitalizadoResponse
        {
            PacienteId = p.PacienteId ?? string.Empty,
            IdIngreso = p.IdIngreso,
            Cedula = p.Cedula,
            TipoDocumento = p.TipoDocumento,
            NombreCompleto = p.NombreCompleto?.Trim() ?? string.Empty,
            Edad = p.Edad ?? 0,
            Sexo = p.Sexo,
            Servicio = p.Servicio ?? "Sin servicio",
            Pabellon = p.Pabellon ?? "Sin pabellón",
            Habitacion = p.Habitacion ?? "Sin habitación",
            Cama = p.Cama,
            FechaIngreso = p.FechaIngreso ?? DateTime.Now,
            DiagnosticoPrincipal = p.DiagnosticoPrincipal,
            EstadoPaciente = p.EstadoPaciente ?? "Desconocido"
        });
    }

    private class PacienteHospitalizadoDto
    {
