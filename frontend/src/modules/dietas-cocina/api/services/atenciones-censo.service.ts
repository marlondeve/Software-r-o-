...
/** Candidatos del HIS vía GET /api/v1/atenciones/hospitalarias */
export async function obtenerCandidatosCensoDesdeAtenciones(
  comida: TiempoComida,
): Promise<Omit<FilaDieta, "id">[]> {
  edadPorDocumento.clear()
  const atenciones = await getAtencionesHospitalarias()
  const edades = await Promise.all(atenciones.map(enriquecerEdadPaciente))

  return atenciones.map((atencion, index) =>
    mapearAtencionHospitalariaAFilaDieta(atencion, comida, {
      edad: edades[index],
    }),
  )
}

/** Fusiona atenciones hospitalarias activas con el estado operativo local. */
export async function sincronizarFilasDesdeAtenciones(
  comida: TiempoComida,
  filasActuales: FilaDieta[] = [],
): Promise<{ filas: FilaDieta[]; totalEnCenso: number }> {
  const candidatos = await obtenerCandidatosCensoDesdeAtenciones(comida)
  return fusionarCensoOperativo(filasActuales, candidatos, comida)
}

/** Sincroniza todas las comidas operativas desde atenciones hospitalarias. */
export async function cargarFilasCensoDesdeAtenciones(
