...
| **RN-DC-019** | Registrar novedad | `FilaDieta` + ciclo | Estado visible ∈ `{guardado, confirmada, por-iniciar, en-preparacion, devuelta}` AND ventana abierta | Permite modificación con motivo | `evaluarAccionesDietaClinica()` | Bloquear desde `lista-despacho`; validar ventana en sheet |
| **RN-DC-020** | Cancelar dieta | `FilaDieta` | Normal: visible `guardado`. Tardía: visible confirmada/en cocina + rol Nutricionista/Doctor/Admin | Cancelación con motivo; tardía requiere checkbox | `evaluarAccionesDietaClinica()` | `POST /dietas/{id}/cancelar` con `CancelarDietaDto` |
