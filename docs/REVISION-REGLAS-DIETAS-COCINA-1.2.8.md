# Revisión de reglas y cambios — Dietas y Cocina v1.2.8

**Versión:** RioSoft 1.2.8 — 2026-08-27  
...
1. [Resumen de lo implementado en 1.2.8](#1-resumen-de-lo-implementado-en-128)
...
## 1. Resumen de lo implementado en 1.2.8

### Nuevo en 1.2.8

| Funcionalidad | Descripción |
|---|---|
| **SignalR tiempo real** | Hub `/hubs/dietas-cocina`: avisa tras `SaveChanges` + trazabilidad/auditoría; no persiste ni duplica historial |
| **Sync HIS en servidor** | `CensoHisSyncHostedService` + candado `sp_getapplock`; un writer; `GET /censo` para carga/botón/fallback |
| **Cliente sin poll 15 s** | Con hub conectado no hay timer de censo; fallback 60 s solo si el socket está caído |
| **409 concurrencia** | Si otro usuario ya cambió el estado: conflicto con DTO actual; UI reemplaza por id/identidad |
| **Refetch pantallas** | Inicio, reportes (vista), auditoría, conciliación, parámetros y permisos al evento |
| **Reloj ventanas 1 s** | Corte de ventana alineado al reloj del hospital |
| **UTC / Colombia** | Instants en UTC; «hoy», códigos de etiqueta y UI en `America/Bogota` |
...
> SignalR **avisa** a las sesiones abiertas. No sustituye `SaveChanges`, `EventoTrazabilidad` ni `EventoAuditoria`. El sync HIS lo hace un `IHostedService` en el servidor (un writer), no cada pestaña.

### 7.1 Sync de censo HIS

| Aspecto | Detalle |
|---|---|
| **Writer periódico** | `CensoHisSyncHostedService` — comidas activas del día Colombia, intervalo `DietasCocina:CensoHisSyncIntervalSeconds` (default **15 s**; **0** desactiva el hosted) |
| **Candado** | `sp_getapplock` por `fecha+comida`, compartido con `GET /censo`. Nunca dos syncs en paralelo (varios workers IIS incluidos) |
| **Endpoint GET** | `GET /api/v1/dietas-cocina/censo?fecha=&comida=` — carga inicial, botón «Actualizar censo» y fallback si SignalR está caído |
| **Push** | Si hubo altas/cancelaciones/reingresos → evento `CensoActualizado` al hub `/hubs/dietas-cocina` |
| **Autor sistema** | `ModificadoPor = "Sistema"` |

**Frontend (un solo origen de refresco):**

- Con SignalR conectado: **cero poll**. No conviven timer 15 s + hub + hosted.
- Fallback **60 s** solo si el socket está desconectado (pestaña visible).
- También: carga inicial, botón, foco/visibilidad si el hub está caído.
- Un `CensoActualizado` no dispara otro GET si ya hay uno en vuelo.
...
**IIS:** el site debe tener **WebSockets** habilitado (`web.config` ya trae `<webSocket enabled="true" />`).

### 7.2 Tiempo real (SignalR) y cocina ← dietas

| Aspecto | Detalle |
|---|---|
| **Hub** | `/hubs/dietas-cocina` (JWT cookie o `access_token` en el query del handshake) |
| **Eventos** | `FilaActualizada`, `CensoActualizado`, `OrdenActualizada`, `EtiquetasActualizadas`, `ParametrosActualizados`, `CatalogoActualizado`, `ConciliacionActualizada`, `PermisosActualizados` |
| **Orden** | Validar → `SaveChanges` → trazabilidad/auditoría → notify. Si el push falla, el dato ya está guardado |
| **UI** | Reemplazo por `id` o identidad (cédula + comida + fecha); nunca concatenar. Deduplicar dietas, órdenes y etiquetas |
| **Concurrencia** | HTTP **409** si otro usuario ya cambió el estado (confirmar/cancelar/reactivar/novedad/orden cancelada) |
| **Cocina ← dietas** | `SincronizarCocinaDesdeDietas.tsx` solo remapea filas → órdenes (sin timer) |
...
| Aviso a otras sesiones | `FilaActualizada` / `OrdenActualizada` por SignalR |
...
- Formularios de solicitud/novedad, Dietas y dashboard nutricionista: timer **1 s** para que el corte de ventana coincida con el reloj del hospital
- No modifica datos; solo UI
- Instants se **guardan en UTC** (`DateTime.UtcNow`); “hoy”/ventanas y textos visibles usan `America/Bogota`

### 7.7 Al arrancar API

- Seed de permisos faltantes en roles de sistema (`RolModuloDefaultsSeed`)
- Hosted service de censo HIS (si el intervalo no es 0)
- Hub SignalR en `/hubs/dietas-cocina`
...
### Sync automático y tiempo real

- [ ] Con SignalR conectado no hay poll de 15 s
- [ ] Egresos/ingresos HIS llegan a todas las pantallas sin pulsar «Actualizar»
- [ ] Fallback 60 s solo si el socket está desconectado
- [ ] Al volver foco a la ventana con hub caído, sync inmediato
- [ ] Órdenes de cocina se actualizan al cambiar filas
- [ ] Dos pestañas no duplican la misma dieta/orden/etiqueta
...
2. **Sync HIS en servidor** — `CensoHisSyncHostedService` + candado SQL; el poll 15 s del navegador ya no es el writer. Fallback 60 s solo si SignalR está desconectado
...
| `Bital.ApiNegocio/Hubs/DietasCocinaHub.cs` | Hub SignalR autenticado |
| `Bital.ApiNegocio/Hosted/CensoHisSyncHostedService.cs` | Sync HIS periódico en servidor |
| `Bital.Infrastructure/DietasCocina/CensoHisAppLock.cs` | Candado `sp_getapplock` por fecha+comida |
...
| `realtime/SincronizarRealtimeDietasCocina.tsx` | Hub SignalR + fallback 60 s |
| `context/SincronizarCocinaDesdeDietas.tsx` | Remapeo filas → órdenes (sin poll) |
| `context/DietasOperativasContext.tsx` | Sync censo, reemplazo por identidad, acciones API |
...
| Suite | Archivos |
|---|---|
| Backend (~50 tests) | `DietasReglasNegocioSalidaClinicaTests.cs`, `ReporteCocinaHelperTests.cs`, `CocinaReporteServiceTests.cs`, `DietasCocinaRealtimeYZonaTests.cs` |
| Frontend (~126+ tests) | `labelEstadoOperativo.test.ts`, `fusionarFilasDieta.test.ts`, `formatoHora.test.ts`, `auditoria.mapper.test.ts` |
...
| 3 | 09:15 | Próximo sync HIS en servidor (~15 s) + `CensoActualizado` | Estado → `Cancelada` |
...
```text
09:00:00  Usuario abre Gestión de dietas (hub SignalR conectado)
09:00:00  GET censo (carga inicial)
09:00:15  Hosted HIS en servidor; si hubo cambios → CensoActualizado → un GET
          (el navegador NO hace poll de 15 s)
09:05:00  Usuario cambia a otra app (pestaña oculta)
09:05:15  Hosted sigue corriendo en servidor; la UI no pide censo extra
09:10:00  Usuario vuelve: si el hub sigue vivo, no hay GET extra; si cayó, fallback/focus
09:10:00  Si IngInSlC=S detectado en el writer del servidor → cancelación/sostenimiento
          y las pantallas abiertas reciben CensoActualizado
```
```
...
- Changelog producto: `CHANGELOG.md` — sección [1.2.8]
- Manual técnico general: `docs/MANUAL-TECNICO.md`
- **Ejemplos y checklist de reglas:** `docs/REVISION-REGLAS-DIETAS-COCINA-1.2.8.md` (§16)
